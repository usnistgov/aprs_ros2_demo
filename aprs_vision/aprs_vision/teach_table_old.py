...
class TeachTable(Node):    
    ...
    def __init__(self):
        super().__init__(node_name='teach_table_vision_node')

        self.capture = cv2.VideoCapture("http://192.168.1.105/mjpg/video.mjpg")
        ...
        self.map_x = np.load(os.path.join(share_path, 'config', 'teach_table_map_x.npy'))
        self.map_y = np.load(os.path.join(share_path, 'config', 'teach_table_map_y.npy'))
        self.base_background = cv2.imread(os.path.join(share_path, 'config', 'teach_table_background.jpg'))
    ...
    def remove_and_replace_gears(self, frame: MatLike, gear_size="large") -> MatLike:
        ...
        if gear_size == "large":
            hsv_lower = (40, 90, 123)
            hsv_upper = (83, 186, 255)
            radius = 60
        elif gear_size == "medium":
            hsv_lower = (0, 80, 170)
            hsv_upper = (25, 175, 255)
            radius = 44
        else:
            return frame
    ...
    def determine_tray_info(self, trays_on_table: dict[int, list[tuple[tuple[int, int], float]]]) -> Trays:       
        ...
        for identifier, trays in trays_on_table.items():
            for i, ((x,y), angle) in enumerate(trays):
                ...
                tray_msg.name = f'{TeachTable.tray_names[identifier]}_{i}'

                tray_center = Point(
                    x=(TeachTable.table_origin.x - (x * self.conversion_factor)) / 1000,
                    y=(TeachTable.table_origin.y + (y * self.conversion_factor)) / 1000, 
                    z=(TeachTable.table_origin.z + self.tray_height)
                )
                ...
                if identifier in TeachTable.part_tray_types:
                    tray_info.part_trays.append(tray_msg)
                elif identifier in TeachTable.kit_tray_types:
